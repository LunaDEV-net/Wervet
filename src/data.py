...
def setup_data(data_dict: dict, const_data: Default, beobachtungs_id, stationsname, beobachtet_von, beobachtet_bis) -> dict:
    ...
    if data_dict.get(beobachtungs_id) == None:  # Wenn es noch kein Eintrag für die BeobachtungsID gibt
        ...
        for i in range(4):
            ...
            # data_dict[beobachtungs_id][i][const_data.POS_BERUFSGRUPPE] = "Berufsgruppe (soll nachher überschrieben werden)"
...
def update_data_dict(data_dict: dict, beobachtungs_id, berufsgruppe, indikator, hd, handschuhe, const_data: Default) -> dict:
...
def process_data(data_in: list, const_data: Default) -> dict:
    ...
    for line in data_in:
        ...
        handschuhe = line[const_data.handschuhe]

        data_dict = setup_data(data_dict, const_data, beobachtungs_id, stationsname, beobachtet_von, beobachtet_bis)
        data_dict = update_data_dict(data_dict, beobachtungs_id, berufsgruppe, indikator, hd, handschuhe, const_data)
